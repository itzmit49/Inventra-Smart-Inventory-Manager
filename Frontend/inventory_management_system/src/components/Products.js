...
export default function Products() {
    ...
    const getProducts = async () => {
        setIsLoading(true);
        try {
            const res = await fetch("https://inventra-smart-inventory-manager.onrender.com/products", {
                method: "GET",
                headers: {
    ...
    const deleteProduct = async (id) => {
        ...
        try {
            const response = await fetch(`https://inventra-smart-inventory-manager.onrender.com/deleteproduct/${id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
